...
| B | `maldongmu-screen` | 화면·스타일·자막 규칙 | `opus` | 판단이 정본이라 `fable`을 배정했으나 사용량 한도로 돌지 못해 교체(2026-09-13) |
| C | `maldongmu-docs` | 문서 정비와 `doc-audit` | `opus` | 절차가 정본. `doc-audit` 스킬이 판정 기준을 이미 들고 있다 |

⚠ **모델 한도에 걸린 세션은 아무 일도 하지 않고 `idle`이 된다.** 2026-09-13 실측: 세션 B가
두 번 연속 착수에 실패했는데 증상이 "창은 살아 있고 `ListAgents`는 `idle`, worktree에 파일 0,
커밋 0"이었다. 이것은 런처가 착수 프롬프트를 첫 입력으로 넘기지 못한 알려진 사고와 **증상이
구별되지 않는다.** 재발송으로도 창 재생성으로도 풀리지 않았고, 원인은 `fable` 사용량 한도였다.

**판별 순서**: 세션이 `idle`인데 결과물이 0이면 ①그 모델의 사용량 한도를 먼저 확인하고
②그다음에 프롬프트 미전달을 의심한다. 한도가 원인이면 창을 몇 번을 다시 띄워도 같다.
