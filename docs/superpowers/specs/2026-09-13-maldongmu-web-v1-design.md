...
- `#copyDiagnostics`: 정지 후 나타나는 「진단 정보 복사」 버튼. 확정 자막이 없어도 나타나며
  다시 시작하면 숨기고 이전 진단을 지운다. 세션 진단은 메모리에만 두며 자동 전송하지 않는다.
- `web/diagnostics.js`가 `app.js`를 통해 인식 사건을 받아 수치만 누적한다. 발화 텍스트,
  오류 메시지, UA 원문은 저장하거나 복사하지 않는다. 인식·자막 모듈 계약은 유지한다.
- 세션 길이는 시작 요청부터 정지 완료까지, 확정 건수는 `final` 사건 수다. 경과 시간은
  `performance.now()`로 재고 날짜는 시작한 기기의 현지 날짜로 표시한다.
- 끊김은 `recovering` 진입 횟수다. 간격은 첫 항목만 시작 요청부터, 나머지는 직전 끊김부터다.
  이미 복구 상태인 동안 생긴 원시 `end` 사건은 현재 인식 계약으로 관찰할 수 없다.
- 재연결 성공은 `listening` 복귀, 실패는 복구 중 자동 종료, 중단은 복구 중 사용자 정지다.
  최대 재연결 공백은 성공한 재연결의 `recovering`부터 `listening`까지다. 성공이 없으면
  「해당 없음」으로 표시한다. 음성 입력 복구를 단정하지 않도록 재연결 후 `final` 또는
  `interim` 수신 횟수를 재연결별로 별도 집계한다. 결과 미수신은 침묵일 수도 있다.
- 오류는 인식 모듈의 `kind`별 빈도다. 원시 `aborted`·`audio-capture` 등은 `unknown`에
  합쳐지므로 원시 오류별 횟수로 해석하지 않는다.
- 브라우저·OS·기기는 UA의 고정 토큰과 버전에서 추출하고, 알 수 없는 값은 「정보 없음」으로
  표시한다. UA가 축약한 OS 버전은 실제 OS 버전과 다를 수 있다. 화면 크기는 CSS 픽셀 기준
  `screen.width/height`, 홈 화면 실행 여부는 `display-mode: standalone`과 iOS `standalone`이다.
- 복사 시 글자 크기와 해당 세션 시작 때 조회한 한국어 온디바이스 가용성을 포함한다.
  `available()`에 `langs: ['ko-KR'], processLocally: true`를 전달한다. 미지원·조회 중·조회 실패·
  정보 없음과 API의 네 가지 결과를 구분하며 팩을 설치하거나 인식 경로를 바꾸지 않는다.
  가용성은 현재 처리 방식의 증거가 아니다([API 문서](https://developer.mozilla.org/en-US/docs/Web/API/SpeechRecognition/available_static)).
- 복사 성공·실패는 기존 `#status` 한 곳에 알린다. 복사 중에는 `aria-disabled`와 입력 가드로
  중복 실행을 막고 버튼 포커스를 유지한다. 새 세션에 이전 복사의 완료 통지를 덮어쓰지 않는다.
